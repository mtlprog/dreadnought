...
const getClaimableBalances = (
  server: Horizon.Server,
  publicKey: string,
): Effect.Effect<unknown[], StellarError> =>
  pipe(
    Effect.tryPromise({
      try: async () => {
        const claimableBalances = await server
            ...
            projectEntries.map(({ code, cid }) =>
              pipe(
                Effect.all([
                  ...
                  pipe(
                    getStellarConfig(),
                    Effect.flatMap(({ publicKey, server }) => checkTokenExists(server, publicKey, code)),
                  ),
                  pipe(
                    getStellarConfig(),
                    Effect.flatMap(({ publicKey, server }) => getClaimableBalances(server, publicKey)),
                  ),
                ]),
                Effect.map(
                  ([projectData, tokenExists, claimableBalances]) => {
                    if (!tokenExists) {
                      return null; // Skip projects without tokens
                    }

                    const supportersCount = countUniqueSupporters(
                      claimableBalances,
                      code,
                    );
                    const currentAmount = calculateRaisedAmount(
                      claimableBalances,
                      code,
                    );
